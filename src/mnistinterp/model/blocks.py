"""Source: https://github.com/tonyduan/diffusion"""
...
class ContinuousPositionalEmbedding(nn.Module):
    ...
    def forward(self, t):
        """Params
        t: (batch_size,) shapez tensor."""

        batch_size = t.shape[0]
        t = t.reshape(batch_size, 1)

        div_term = torch.exp(
            torch.arange(0, self.embedding_size, 2, device=t.device)
            * (-math.log(self.max_len / 2 / math.pi) / self.embedding_size)
        ).unsqueeze(0)

        embedding = torch.zeros(batch_size, self.embedding_size, device=t.device)
        embedding[:, : self.embedding_size // 2] = torch.sin(t * div_term)
        embedding[:, self.embedding_size // 2 :] = torch.cos(t * div_term)
...
class UNet(nn.Module):
    ...
    def __init__(self, in_dim, embed_dim, out_dim, dim_scales):
        super().__init__()

        self.init_embed = nn.Conv2d(in_dim, embed_dim, 1)
        self.time_embed = ContinuousPositionalEmbedding(embed_dim, max_length=50)
        # I set max_lenght manually by plotting what the embeddings look like with
        # time values ranging from -4 to 4, which is the range of values we get
        # with the Karras formulation.
